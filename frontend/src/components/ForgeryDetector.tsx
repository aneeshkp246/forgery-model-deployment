...
const ForgeryDetector = () => {
  ...
  const predictSingle = async (imageData: ImageData, index: number): Promise<PredictionResult> => {
    ...
    try {
      ...
      if (typeof data.confidence === 'number' && data.confidence < 0.05) {
        ...
        processedResult.confidence = 1 - data.confidence;
      }
  ...
  const handleSinglePredict = async () => {
    ...
    updatedImages[currentIndex] = { ...currentImage, loading: true };
    ...
    try {
      ...
      const finalUpdatedImages = [...images];
      finalUpdatedImages[currentIndex] = { ...currentImage, loading: false };
      setImages(finalUpdatedImages);
  ...
  const handleBatchPredict = async () => {
    ...
    updatedImages.forEach((img, index) => {
      updatedImages[index] = { ...img, loading: true };
    });
    setImages(updatedImages);
  ...
  const downloadCSV = () => {
    ...
    const csvHeaders = ['Image Name', 'Prediction', 'Confidence (%)'];
    const csvRows = images.map((imageData, index) => {
      const fileName = imageData.file.name;
      const prediction = imageData.result?.label || 'Not processed';
      const confidence = imageData.result?.confidence 
        ? (imageData.result.confidence * 100).toFixed(1)
        : 'N/A';
            ...
            <div className="bg-gray-50 rounded-xl p-6 border-l-4 border-blue-500">
              <div className="text-center space-y-4">
                <h2 className="text-2xl font-bold text-gray-800">
                  Detection Result - Image {currentIndex + 1}
                </h2>
                
                <div className={`inline-flex items-center px-4 py-2 rounded-full text-lg font-semibold ${
                  currentImage.result.label === 'Real' 
                    ? 'bg-green-100 text-green-800 border-2 border-green-200' 
                    : 'bg-red-100 text-red-800 border-2 border-red-200'
                }`}>
                  <span className="mr-2">
                    {currentImage.result.label === 'Real' ? '✅' : '⚠️'}
                  </span>
                  {currentImage.result.label}
                </div>

                {typeof currentImage.result.confidence === 'number' && (
                  <div className="space-y-3">
                    <p className="text-gray-700 font-medium">
                      Confidence: <span className="font-bold">{(currentImage.result.confidence * 100).toFixed(1)}%</span>
                    </p>
                    
                    <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                      <div 
                        className={`h-full rounded-full transition-all duration-500 ${
                          currentImage.result.label === 'Real' ? 'bg-green-500' : 'bg-red-500'
                        }`}
                        style={{ width: `${currentImage.result.confidence * 100}%` }}
                      ></div>
                    </div>
                    
                    <div className="text-sm text-gray-500">
                      {currentImage.result.confidence > 0.8 ? 'High confidence' : 
                       currentImage.result.confidence > 0.6 ? 'Medium confidence' : 'Low confidence'}
                    </div>
                  </div>
                )}
              ...
              <div className="text-center">
                ...
                <div className="w-full bg-blue-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ 
                      width: `${(images.filter(img => img.result || !img.loading).length / images.length) * 100}%` 
                    }}
                  ></div>
                </div>
                <p className="text-sm text-blue-600 mt-1">
                  {images.filter(img => img.result).length} of {images.length} completed
                </p>
              </div>
            </div>
          )}
